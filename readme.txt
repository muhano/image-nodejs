...
3. Push the image to dockerhub so kubernetes can pull it:
...
4. create .yaml file to deploy it in kubernetes/minikube, for example this is .yaml for create pod:
```
apiVersion: v1
kind: Pod
metadata:
  ...
  containers:
    - name: image-nodejs
      ...
      ports:
        - containerPort: 3000
```
5. deploy it in kubernetes by running the command "kubectl create -f pod-nodejs.yaml"